...
export const DeckBuilder: React.FC<DeckBuilderProps> = ({
    ...
    const availableCards = useMemo(() => {
        return Object.values(CARD_DB).filter(c => {
            // ... (省略部分过滤逻辑) ...
            //if (['lyfe_rush', 'lyfe_ultimate', 'fenny_strike', 'fenny_ultimate'].includes(c.key)) return false;

            if (filterRegion === 'ALL') return true;
            if (filterRegion === 'LYFE') return c.region === 'Lyfe';
            if (filterRegion === 'FENNY') return c.region === 'Fenny';
            if (filterRegion === 'LOGISTICS') return c.region === 'Logistics';
            if (filterRegion === 'TEST') return c.region === 'TEST';
    ...
    // [新增] 一键填满函数
    const fillCard = (key: string) => {
        const card = CARD_DB[key];
        const currentCount = deck[key] || 0;

        // 1. 计算单卡还需要几张
        const isTestCard = key.startsWith('test_');
        const maxPerCard = isTestCard ? 99 : 3; // 测试卡无上限，普通卡3张
        const needed = maxPerCard - currentCount;

        if (needed <= 0) return; // 已经满了

        // 2. 计算全局还能放几张
        const globalRemaining = 40 - stats.total;
        let actualToAdd = Math.min(needed, globalRemaining);

        // 3. 如果是英雄，检查英雄位还能放几张
        if (card.isChampion) {
            const championRemaining = 6 - stats.champions;
            actualToAdd = Math.min(actualToAdd, championRemaining);
        }

        if (actualToAdd <= 0) return;

        // 4. 执行更新
        eventBus.emit(GameEvents.DECK_ADD_CARD); // 播放一次音效即可 (或者你想播多次也可以)
        setDeck(prev => ({
            ...prev,
            [key]: (prev[key] || 0) + actualToAdd
        }));
    };
    ...
    const addCard = (key: string) => {
        ...
        // 检查限制
        if (stats.total >= 40) return; // 总数限制
        if (card.isChampion && stats.champions >= 6) return; // 英雄限制

        // [修改] 凡是以 'test_' 开头的卡牌，都不受 3 张限制
        const isTestCard = key.startsWith('test_');
        if (!isTestCard && currentCount >= 3) return;

        eventBus.emit(GameEvents.DECK_ADD_CARD);
        setDeck(prev => ({ ...prev, [key]: currentCount + 1 }));
    };

    const removeCard = (key: string) => {
        if (!deck[key]) return;
        eventBus.emit(GameEvents.UI_CLICK); // <--- 移除时触发普通点击音效 (也可以是撤回音效，看你喜好，这里暂用点击)
        setDeck(prev => {
            const next = { ...prev };
